...
try:
 ...
 call([R/'run_control.py','smoke','0'],'smoke.log')
 assert json.loads((R/'SMOKE_RESULT.json').read_text())['status']=='PASS'
 status('FORMAL_RUNNING')
 logs=[]
 for w in range(2):
  f=(R/f'formal_{w}.log').open('a');logs.append(f)
  children.append(subprocess.Popen([sys.executable,str(R/'run_control.py'),'formal',str(w)],stdout=f,stderr=subprocess.STDOUT,cwd=R))
 ...
 call([R/'run_control.py','integrity'],'integrity.log')
 call([R/'summarize_new.py'],'statistics.log')
 ...
 try:call([R/'run_control.py','integrity'],'integrity.log');call([R/'summarize_new.py'],'statistics.log')

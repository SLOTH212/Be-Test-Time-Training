...
def summarize(root):
    ...
    for c in expected:
        ...
        for b in r['branches']:
            ...
            for s in b['seeds']:
                ...
                norm_valid = bool(np.isfinite(err) and abs(err - dg['relative_norm_error']) <= 1e-9 and dg['realized_norm'] > 0 and dg['target_norm'] > 0)
                algorithm = 'gaussian_bf16_scalar_bracket_binary_v1'
                payload = json.dumps([algorithm, int(s['seed']), str(sid), int(layer)], ensure_ascii=False, separators=(',', ':')).encode('utf-8')
                digest = hashlib.sha256(payload).digest()
                construction_valid = (
                    dg.get('algorithm_version') == algorithm and dg.get('seed_payload_sha256') == digest.hex()
                    and dg.get('subseed') == (int.from_bytes(digest[:8], 'big') & ((1 << 63) - 1))
                    and dg.get('reward_used') is False and dg.get('execution_dtype') == 'bfloat16'
                    and dg.get('norm_dtype') == 'float32' and dg.get('represented_nonzero') is True
                    and dg.get('requested_norm') == dg['target_norm']
                    and dg.get('calibration_iterations') == 1 + dg.get('bracket_steps', -100) + dg.get('binary_steps', -100)
                    and dg.get('matched_within_1pct') == (err <= .01))
                exception_valid = (dg.get('bracketed') is True and dg.get('binary_steps') == 32
                    and dg.get('max_binary_steps') == 32 and 0 <= dg.get('bracket_steps', -1) <= 24
                    and dg.get('max_bracket_steps') == 24 and dg.get('search_relative_tolerance') == .001)
                closest_validation.append(norm_valid and construction_valid and (err <= .01 or exception_valid))
                if err > .01:
                    closest_exceptions.append(dict(sample_id=sid, layer=layer, seed=s['seed'], relative_norm_error=err,
                        closest_fallback_validated=bool(norm_valid and construction_valid and exception_valid)))
    ...
    integrity = source.get('status') == 'PASS' and source.get('amendment_checked') is True and (root / 'PROTOCOL_AMENDMENT_01.json').is_file()
    ...
    closest_valid = bool(closest_validation) and all(closest_validation)
    strict_within_1pct = checks['A9']
    magnitude_notes = [e for e in errors if e.startswith('Magnitude mismatch:') or e.startswith('A9 ')]
    if closest_valid:
        errors = [e for e in errors if e not in magnitude_notes]
    checks['A9'] = closest_valid
    final_integrity = complete and integrity and closest_valid and all(v for k, v in checks.items() if k != 'A9')
    ...
    summary = dict(status=status, completed_n=len(rows), expected_n=157, checks=checks,
                   errors=errors, source_integrity=integrity, final_integrity=final_integrity, protocol_closest_fallback_validated=closest_valid, strict_within_1pct=strict_within_1pct, magnitude_notes=magnitude_notes, closest_exceptions=closest_exceptions, A1_frozen_historical=historical_a1,
    ...
    contract = dict(MAGNITUDE_MATCHED_CONTROL_STATUS=status, COHORT_N=157, COHORT_EXACT=checks['A1'], N_SEEDS=len(seeds),
        ...
        RANDOM_MINUS_DELETION_PP=a3.get('mean_pp'), RANDOM_MINUS_DELETION_CI95=a3.get('CI95'), FINAL_INTEGRITY='PASS' if final_integrity else 'FAIL', FORMAL_COMPLETE=complete, PROTOCOL_CLOSEST_FALLBACK_VALIDATED=closest_valid, STRICT_WITHIN_1PCT=strict_within_1pct)
    ...
    if status == 'PASS':
        text += f"Magnitude policy: the user permits the closest evaluated deterministic scalar when BF16 quantization misses 1%. CHECK_A9 validates that policy; STRICT_WITHIN_1PCT separately reports the numerical target. Exceptions: {len(closest_exceptions)}; maximum relative error {mag['max_relative_error']}; within-1% fraction {mag['matched_within_1pct_fraction']}. No seeds, directions, cohort or calibration algorithm changed. Closest means best among evaluated scalar candidates, not proven global scalar optimum.\n\n"
        if not strict_within_1pct:
            text += 'Interpretation is conditional on the permitted closest-match BF16 control and its reported residual norm errors; this is numerically limited evidence, not an execution failure or a claim that every norm is within 1%.\n\n'
        lo, hi = a3['CI95']
        if lo > 0:
            text += 'CASE 1 under the stated closest-match policy: Matched random exceeds deletion, supporting greater disruption from deleting the trajectory-formed offset than from these approximately matched nonzero perturbations. Residual norm mismatch must accompany this conclusion; it does not establish a unique causal mechanism.\n'
        elif hi < 0:
        ...
        text += 'No formal scientific conclusion is issued while completion or protocol/source integrity gates are pending/failed.\n'
    text += '\nAudit checks:\n\n' + '\n'.join(f'- CHECK_{k}: {"PASS" if v else "FAIL/PENDING"}' for k,v in checks.items())+'\n'
    (root / 'final_report.md').write_text(text)
    (root / 'audit_report.md').write_text('# Independent CPU audit\n\n'+json.dumps(dict(status=status, checks=checks, source_integrity=integrity, final_integrity=final_integrity, protocol_closest_fallback_validated=closest_valid, strict_within_1pct=strict_within_1pct, magnitude_notes=magnitude_notes, closest_exceptions=closest_exceptions, completed=len(rows), errors=errors), indent=2)+'\n\nNo existing Selective Deletion results were modified. A12 requires explicit reward-blind source-review evidence in config or source integrity check.\n')
    return summary

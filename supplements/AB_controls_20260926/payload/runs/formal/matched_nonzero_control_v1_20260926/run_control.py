 ...
 for layer in c['deletion_targets']:
  ...
  for seed in cfg['seeds']:
   ...
   if not diag['matched_within_1pct']:
    put(O/'magnitude_failure.json',dict(sample_id=c['sample_id'],layer=layer,seed=seed,diagnostics=diag))
...
if __name__=='__main__':
 ...
 try:
  ...
  elif phase=='integrity':integrity();put(O/'source_integrity_check.json',dict(status='PASS',historical_unchanged=True,experiment_sources_unchanged=True,reward_blind_construction=True))
  else:run(phase,worker)
 except Exception as e:
  put(O/f'FAILURE_{phase}_{worker}.json',dict(status='FAIL',phase=phase,error=str(e),traceback=traceback.format_exc()))

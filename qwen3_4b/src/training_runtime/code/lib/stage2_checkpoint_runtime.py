"""Stage2 DCP restore adapter preserving absent lazy optimizer state."""
import json
import torch
from torch.distributed.checkpoint import FileSystemReader
from .distributed_checkpoint import DistributedCheckpointRotation as BaseRotation
class Stage2CheckpointRotation(BaseRotation):
 def load(self,model,optimizer,scheduler=None):
  ...
  self.stage2_lazy_restore_audit={'saved_state_count':len(saved_names),'removed_initialization_only_state':extra,'scientific_state_modified':False}
  print(json.dumps({'event':'STAGE2_LAZY_OPTIMIZER_RESTORE',**self.stage2_lazy_restore_audit}),flush=True)
  return progress

...
ROOT = Path(r"C:\datasets\qa_replay_30m_32k_qwen3_v2")
HROOT = ROOT / "recovered_authority/root/autodl-tmp"
...
def main():
            ...
            for line_no, header, raw in iter_mrqa(source):
                ...
                for qa in raw.get("qas", []):
                    ...
                    if qid in expected_lookup[source]:
                        ...
                        # strong raw-byte identity oracle.
    ...
    builder_files = [ROOT / "builder/build_v2.py", ROOT / "builder/independent_audit.py"]
    builder_authority = {"python": sys.version, "platform": platform.platform(),
        ...
        "command": f"PYTHONPATH=<frozen runtime_pkgs> python {ROOT / 'builder/build_v2.py'}",

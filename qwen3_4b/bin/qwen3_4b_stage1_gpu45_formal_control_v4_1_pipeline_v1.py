...
NAME="qwen3_4b_stage1_1b_32k_gpu45_formal_v1"
...
def query(fields,apps=False):
 ...
 p=subprocess.run(["nvidia-smi","-i","4,5",q+fields,"--format=csv,noheader,nounits"],capture_output=True,text=True,check=True,timeout=15)
...
def guard():
 ...
 if os.environ.get("CUDA_VISIBLE_DEVICES")!="4,5" or os.environ.get("WORLD_SIZE")!="2":raise RuntimeError("GPU_POLICY_VIOLATION")
 ...
 if digest(CFG)!="d7180bea369e128635289016568654fb592b0f995474a8532e45d473bb3454db":raise RuntimeError("FROZEN_FORMAL_CONFIG_CHANGED")
 global WORKER
 WORKER=owned('/path/to/ttt/bin/stage1_runtime_entry_gpu45_pipeline_v1.py')
 if digest(WORKER)!=resource["files"].get(str(WORKER)):raise RuntimeError("FORMAL_ENTRYPOINT_NOT_BOUND")
 import shutil
 ...
 if cfg["world_size"]!=2 or cfg["physical_gpu_allowlist"]!="4,5" or cfg["global_batch_size"]!=cfg["micro_batch_size"]*2*cfg["gradient_accumulation"]:raise RuntimeError("FORMAL_BATCH_GPU_EQUATION")
 ...
 if [int(r[0]) for r in gpus]!=[4,5]:raise RuntimeError("PHYSICAL_GPU_SET_MISMATCH")
 for r in gpus:
  i=int(r[0])

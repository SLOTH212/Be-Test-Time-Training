...
R=Path('/path/to/ttt')
CONFIG=R/'configs/qwen3_4b_stage2_30m_32k_gpu23_formal_v1.yaml'
...
def check(parent):
 ...
 for key,p in [('stage2_checkpoint_adapter_sha256',R/'src/training_runtime/code/lib/stage2_checkpoint_runtime.py'),('stage2_entry_sha256',R/'bin/stage2_runtime_entry_v1.py')]:
 ...
 if os.environ.get('CUDA_VISIBLE_DEVICES')!='2,3':raise RuntimeError('GPU_ALLOWLIST')
 gpu=subprocess.check_output(['nvidia-smi','-i','2,3','--query-gpu=index,memory.free','--format=csv,noheader,nounits'],text=True)
 for line in gpu.splitlines():
  idx,free=map(int,line.split(','))
  if idx not in (2,3) or free<48000:raise RuntimeError('GPU_MEMORY_CAPACITY_NOT_AVAILABLE')
 return a,cfg
def status():
 ...
 out['gpu23']=subprocess.check_output(['nvidia-smi','-i','2,3','--query-gpu=index,memory.used,power.draw','--format=csv'],text=True)
...
def main():
 ...
 p=subprocess.Popen(['torchrun','--standalone','--nproc-per-node=2',str(R/'bin/stage2_runtime_entry_v1.py'),'--stage','2','--config',str(run_cfg),'--output-dir',str(root),'--parent-artifact',a['checkpoint_path']],stdout=log,stderr=subprocess.STDOUT,start_new_session=True)

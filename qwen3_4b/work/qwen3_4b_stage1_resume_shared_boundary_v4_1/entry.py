...
v4_audit=R/'audits/qwen3_4b_stage1_resume_shared_boundary_v4_1'
v4_protocol=v4_audit/'MISSING_CAPTURE_CLOSURE_PROTOCOL_V4_1.json'
...
promoted_keys=['post_step.optimizer.model.layers.18.mlp.ttt_ntp_gate.exp_avg', 'post_step.optimizer.model.layers.24.mlp.ttt_ntp_gate.exp_avg', 'post_step.optimizer.model.layers.30.mlp.ttt_ntp_gate.exp_avg_sq'];selected={k[len("post_step.optimizer."):].rsplit(".",1)[0] for k in promoted_keys};captured={};capture_meta={};capture_events=[];forward_trace=[];forward_active=False;clip_observation={};observer_read_checks=0
out=Path(sys.argv[sys.argv.index('--output-dir')+1]).resolve();assert out.is_relative_to(R/'runs/qwen3_4b_stage1_resume_shared_boundary_v4_1');out.mkdir(parents=True,exist_ok=True)
...
def capture_state(stage,parameters=False,moments=False,gradients=False):
 assert stage=='post_step' and live.step==3
 params=dict(live.model.named_parameters())
 for key in promoted_keys:
  name,state=key[len('post_step.optimizer.'):].rsplit('.',1)
  value=live.optimizer.state[params[name]][state]
  assert value is not None and list(value.shape)==[2560]
  capture_value(key,value,'optimizer_state')
  capture_meta[key]['placement']=str(getattr(value,'placements',None))
  assert capture_meta[key]['local_shape']==[1280] and not capture_meta[key]['replicated']
...
class AuditRotation(Stage1ClosureCheckpointRotation):
 ...
 def save(self,model,optimizer,progress,scheduler=None):
  ...
  # Diagnostic-only final export of exactly three promoted states; no final DCP.
  names={id(p):n for n,p in model.named_parameters()}
  data=captured;meta=capture_meta
  assert set(data)==set(promoted_keys) and set(meta)==set(promoted_keys)
  assert {x['stage'] for x in capture_events}=={'post_step'}

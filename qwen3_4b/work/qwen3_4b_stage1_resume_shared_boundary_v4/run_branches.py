...
R=Path('/path/to/ttt');W=R/'work/qwen3_4b_stage1_resume_shared_boundary_v4';A=R/'audits/qwen3_4b_stage1_resume_shared_boundary_v4';RUN=R/'runs/qwen3_4b_stage1_resume_shared_boundary_v4';RUN.mkdir(exist_ok=True)
sha=lambda p:hashlib.sha256(Path(p).read_bytes()).hexdigest()
P=A/'SHARED_BOUNDARY_CAUSAL_PROTOCOL_V4.json';H=sha(P);plan=json.loads(P.read_text());prior=json.loads((A/'PRIOR_AUTHORITIES_IMMUTABLE.json').read_text())['files'];assert sha(W/'entry.py')==plan['observer_sha256']
freeze=json.loads((A/'PROTOCOL_FREEZE_RECEIPT.json').read_text());assert freeze['sha256']==H
branches=['A','B1','B2'] if len(sys.argv)==1 else ['B3']
if branches==['B3']:
 decision=json.loads((A/'ABC_DECISION.json').read_text());assert decision['run_B3'] and decision['protocol_sha256']==H
...
for branch in branches:
 ...
 out=RUN/branch;log=R/'logs'/('qwen3_4b_stage1_shared_boundary_v4_'+branch+'.log');assert not out.exists() and not log.exists()

...
R=Path('/path/to/ttt');H=R/'work/qwen3_4b_stage2_preflight_v1/debug_stage2_hf_4k_v3';A=R/'audits/qwen3_4b_stage2_30m_32k_gpu23_preflight_v1'
...
sys.path.insert(0,str(R/'src/inference_runtime_4k_stage2_v1/code'))
...
for mode in ['OFF','L0']:
 ...
 try:
  ...
  if mode=='L0':assert len(stats)==1 and stats[0].update_count==1 and stats[0].valid_tokens==4095
  assert all(p._version==before[n] for n,p in model.named_parameters())
  results.append({'mode':mode,'status':'PASS','prompt_tokens':ids.shape[1],'generated_tokens':1,'ttt_stats_count':len(stats),'ttt_valid_pair_counts':[s.valid_tokens for s in stats],'ttt_update_counts':[s.update_count for s in stats],'elapsed_seconds':time.perf_counter()-started,'peak_memory_bytes':torch.cuda.max_memory_reserved(),'physical_gpu':3,'parameter_versions_unchanged':True})
 finally:
...
receipt={'status':'PASS','DEBUG_ONLY':True,'PAPER_RESULT':False,'checkpoint':str(H),'loading_info':info,'results':results,'runtime':'ttt_inference_runtime_4k_stage2_v1_USER_AMENDMENT','runtime_package_sha256':'65a0d7bd9c8801c92fc30477ab8662d3eb52261612eff2acef0bbeb3edf6ba51','benchmark_run':False,'training_chunk':4096,'inference_chunk':4096,'inference_lr':cfg.ttt_lr,'inference_delta_clip':cfg.ttt_update_clip_norm}

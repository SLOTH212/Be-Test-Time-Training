...
import torch
R=Path('/path/to/ttt');L=R/'audits/v3/localization_final';RUN=R/'runs/qwen3_4b_stage1_resume_v3/localization_final'
torch.set_num_threads(8);assert not torch.cuda.is_initialized()
plan=json.loads((L/'CONDITIONAL_LOCALIZATION_PLAN.json').read_text());results=[]
for name in ['U_LOC','R_LOC']:

...
R=Path('/path/to/ttt');A=R/'audits/v3/execution2';W=R/'work/qwen3_4b_stage1_resume_v3';RUN=R/'runs/qwen3_4b_stage1_resume_v3/attempt2';RUN.mkdir(parents=True,exist_ok=True)
P=A.parent/'RESUME_NUMERICAL_EQUIVALENCE_PROTOCOL_V3.json';sha=lambda p:hashlib.sha256(Path(p).read_bytes()).hexdigest();H=sha(P);protocol=json.loads(P.read_text());assert H==P.with_suffix('.sha256').read_text().split()[0]
receipts=[]
def phase(replica,role):
 assert sha(P)==H
 amendment=json.loads((A.parent/'OBSERVER_FIX1_AMENDMENT.json').read_text())
 for path,digest in amendment['new_file_sha256'].items():assert sha(path)==digest
 for path,digest in protocol['input_file_sha256'].items():assert sha(path)==digest,('FROZEN_INPUT_CHANGED',path)
 out=RUN/(replica+'_pre' if role=='R_PRE' else replica);log=R/'logs'/('qwen3_4b_stage1_resume_v3_attempt2_'+replica+'_'+role+'.log');assert not out.exists() and not log.exists()
 ...
 cmd=[str(R/'envs/ttt_runtime_v1/bin/torchrun'),'--standalone','--nproc-per-node=2',str(W/'replicate_entry_observer_fix1.py'),'--stage','1','--config',str(W/'original_replicate_config.yaml'),'--output-dir',str(out),'--max-records',str(records)]

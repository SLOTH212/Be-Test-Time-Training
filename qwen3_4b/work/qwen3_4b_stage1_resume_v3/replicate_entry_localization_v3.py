...
localization_plan=json.loads((A/'CONDITIONAL_LOCALIZATION_PLAN.json').read_text())
assert hashlib.sha256((A/'CONDITIONAL_LOCALIZATION_PLAN.json').read_bytes()).hexdigest()==os.environ['V3_LOCALIZATION_PLAN_SHA256']
selected=set(localization_plan['parameters']);captured={};capture_meta={};capture_events=[]
...
def count_window(window,*args,**kwargs):
 ...
 if live.step==2:capture_state('pre_forward',parameters=True,moments=True)
...
def observed_clip(*args,**kwargs):
 ...
 if targeted:capture_state('pre_clip',gradients=True)

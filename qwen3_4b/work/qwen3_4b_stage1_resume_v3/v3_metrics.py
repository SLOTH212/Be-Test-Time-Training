...
manifest = {'names':names,'pairs':[[names[i],names[j]] for i,j in pairs], 'source_files':[], 'protocol_sha256':hashlib.sha256((A/'RESUME_NUMERICAL_EQUIVALENCE_PROTOCOL_V3.json').read_bytes()).hexdigest()}
for n in names:
 for rank in (0,1):
  p=RUN/n/f'final_rank{rank}.pt';h=hashlib.sha256()
  with p.open('rb') as f:
   for b in iter(lambda:f.read(32<<20),b''):h.update(b)
  assert h.hexdigest()==meta[n][rank]['sha256'] and p.stat().st_size==meta[n][rank]['bytes']
  manifest['source_files'].append({'path':str(p),'sha256':h.hexdigest(),'bytes':p.stat().st_size})
  print('SOURCE_HASH_PASS',n,rank,flush=True)
(OUT/'PAIR_METRIC_MANIFEST.json').write_text(json.dumps(manifest,indent=2)+'\n')
  ...
  for rank in shards:
   ...
   for start in range(0,arrays[0].numel(),1<<20):
    ...
    for p,(i,j) in enumerate(pairs):
     ...
     ps[p,4]+=np.count_nonzero(~torch.isclose(torch.from_numpy(xs[i]),torch.from_numpy(xs[j]),rtol=.01,atol=atol).numpy())
     ps[p,5]+=np.count_nonzero(~torch.isclose(torch.from_numpy(xs[j]),torch.from_numpy(xs[i]),rtol=.01,atol=atol).numpy())

...
active=(ROOT/'src/training_runtime/code/lib/distributed_checkpoint.py').read_text()
helpers=active[active.index('"""Process-crash-safe'):active.index('class DistributedCheckpointRotation:')]
...
for case in ['partial_write','before_backup','after_backup','after_replacement','after_pointer','recovery_interrupted','successful_commit','invalid_pointer','first_uncommitted','corrupt_partial','invalid_pointer_without_backup']:
 root=WORK/'retention_fault_fixtures_integrated_v3'/case
...
report={'status':'PASS','scope':'process-crash publication/recovery; no hardware power-loss claim','cases':results,'latest_and_previous_retained_until_commit':True,'recovery_idempotent':True,'integrated_runtime_helpers_tested':True,'checkpoint_module_sha256':hashlib.sha256((ROOT/'src/training_runtime/code/lib/distributed_checkpoint.py').read_bytes()).hexdigest()}

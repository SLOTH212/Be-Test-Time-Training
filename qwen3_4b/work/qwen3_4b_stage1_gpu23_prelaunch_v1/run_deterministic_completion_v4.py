...
try:
 assert json.loads((audit/"DETERMINISM_PROBE_COMPLETED.json").read_text())["status"]=="PASS"
 ...
 # Reuse the already completed independent fresh two-step trial with identical config.
 before=json.loads((runs/"fresh_B/checkpoints/latest.json").read_text())
 assert before["progress"]["update_step"]==2
 train("RESUME_B_AFTER","fresh_B",6,["--resume-from",str(runs/"fresh_B/checkpoints")])
 compare("REAL_RESUME_NUMERIC_PARITY",anchor,runs/"fresh_B/checkpoints")
 ...
 train("EDGE_ONE_TOKEN","edge_one_token",2,["--debug-start-record","15220"],cfg=work/"real_edge_deterministic_v4.yaml")
 train("EDGE_ODD_FINAL","edge_odd_final",1,["--debug-start-record","38334"],cfg=work/"real_edge_deterministic_v4.yaml")

...
if worker:
 source=replace_exact(source,"os.environ.get('CUDA_VISIBLE_DEVICES')=='4,5' and args.worker_id in [0,1]","os.environ.get('CUDA_VISIBLE_DEVICES')=='0,2,3,4,5' and args.worker_id in [0,1,2,3,4]")
 source=replace_exact(source,"['ae0edae4-c3bc-cfb2-4b15-f8cdd888bc25','25896d0f-c552-9793-5c7a-ced116606c44']","['c7982600-dea0-9d54-16b8-76db4288663e','fb7ba9a8-ce12-65a4-a9cf-5b5b7506b0ed','351e23d4-250b-6a70-2c66-d2fb0221a25a','ae0edae4-c3bc-cfb2-4b15-f8cdd888bc25','25896d0f-c552-9793-5c7a-ced116606c44']")
 source=replace_exact(source,"owned=j['sample_ids'][args.worker_id::2]","owned=j['sample_ids'][args.worker_id::5]")
 source=replace_exact(source,"workspace=root/'workers'/str(args.worker_id)","workspace=root/'workers'/('scale5_'+str(args.worker_id))")
 source=replace_exact(source,'physical_gpu=args.worker_id+4','physical_gpu=[0,2,3,4,5][args.worker_id]')
else:raise RuntimeError('SCALE5_WORKER_ONLY')

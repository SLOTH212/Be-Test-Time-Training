...
sys.path.insert(0, str(CLOSURE/'scripts'))
import closure_core as c

ROOT = Path('/home/USER/ttt_mechanism_analysis_v1/trajectory_closure_factorial_intervention_v1')
LAYERS = c.LAYERS
ACTION_LAYERS = c.ACTION_LAYERS
th = c.th
base = c.base
donor_prefix = c.donor_prefix
load = c.load

def native_used_layers(A, B):
    return sorted(set(ACTION_LAYERS[A]) | set(ACTION_LAYERS[B]))

def intended_fast_hashes(model, native_boundary):
    return {l: th(native_boundary[l] if l in native_boundary else base(model,l)) for l in LAYERS}

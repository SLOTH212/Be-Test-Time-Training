...
R=Path('/path/to/ttt');W=R/'work/llama31_8b_stage2_gpu01234_formal_v3';C=R/'src/training_runtime/code'
assert os.environ['CUDA_VISIBLE_DEVICES']=='0,1,2,3,4' and int(os.environ['WORLD_SIZE'])==5
assert int(os.environ['LOCAL_RANK']) in range(5)
sys.path[:0]=[str(W),str(C),str(R/'src/llama31_ntp_port_v1'),str(R/'src/llama31_ntp_port_v1/source')]
stage=int(sys.argv[sys.argv.index('--stage')+1]);assert stage==2

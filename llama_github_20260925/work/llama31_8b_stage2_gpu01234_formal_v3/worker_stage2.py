...
ROOT = Path('/path/to/ttt/src/training_runtime/code')
...
def main():
    ...
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "0,1,2,3,4":
        raise RuntimeError("PREINIT_PHYSICAL_GPU_ALLOWLIST_VIOLATION")
    if int(os.environ.get("WORLD_SIZE", "0")) != 5 or int(os.environ.get("LOCAL_RANK", "-1")) not in (0, 1, 2, 3, 4):
    ...
    if args.stage != 2 or args.debug_forward_only:
        raise RuntimeError("SEQUENTIAL_STAGE2_TRAINING_ONLY")
    context = initialize()
    try:
        ...
        allowlist = str(cfg.get("physical_gpu_allowlist", "0,1,2,3,4"))
        ...
        while True:
            ...
            # The two weighted objectives accumulate before one optimizer step.
            # Release the answer graph before creating the context graph.
            from sequential_loss import backward_terms
            for slot in range(plan.gradient_accumulation):
                ...
                if len(rows) > 1:
                    raise RuntimeError("SEQUENTIAL_MICROBATCH_ONE_REQUIRED")
                real = bool(rows)
                row = rows[0][1] if real else window[0][1]
                if real:
                    local_sample_ids.append(row.get("sample_id", row.get("id")))
                tokenization_started = time.perf_counter()
                ids = torch.tensor([row["input_ids"]], dtype=torch.long, device=context.device)
                tokenizer_seconds += time.perf_counter() - tokenization_started
                declared = int(row["n_tokens"])
                if ids.shape[1] != declared or declared > int(cfg["context_length"]):
                    raise RuntimeError("TOKEN_ALIGNMENT_ERROR")
                a, c = masks_for(row, ranges)
                wa = context.world_size / global_answer if real else 0.0
                wc = float(cfg["context_group_weight"]) * context.world_size / global_context if real else 0.0
                value = backward_terms(model, ids, a.to(context.device), c.to(context.device), wa, wc, fused_sum)
                if real:
                    local_numerator += value

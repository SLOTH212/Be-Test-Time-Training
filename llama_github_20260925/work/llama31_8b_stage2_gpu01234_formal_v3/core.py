...
R=Path('/path/to/ttt');W=R/'work/llama31_8b_stage2_gpu01234_formal_v3'
CONFIG=Path('/path/to/ttt/configs/llama31_8b_stage2_pipeline_gpu01234_formal_v3.json')
SOURCE=Path('/path/to/ttt/provenance/LLAMA31_8B_STAGE2_GPU01234_SOURCE_V3.json')
...
EXPECTED={1:{'record_cursor':17015,'cumulative_tokens':478374652,'update_step':4254},2:{'record_cursor':457,'cumulative_tokens':14390540,'update_step':92,'cumulative_answer_positions':97619,'cumulative_context_positions':14292464}}
...
def cfg_contract(c,stage):
 ...
 require(c['world_size']==(4 if stage==1 else 5) and c['global_batch_size']==(4 if stage==1 else 5) and c['micro_batch_size']==c['gradient_accumulation']==1 and c['physical_gpu_allowlist']==('0,1,2,3' if stage==1 else '0,1,2,3,4'),'GPU_BATCH_CONTRACT')
...
def gpu():
 expected=['GPU-c7982600-dea0-9d54-16b8-76db4288663e','GPU-8459f05d-4d22-5b39-cc1f-e25137d49389','GPU-fb7ba9a8-ce12-65a4-a9cf-5b5b7506b0ed','GPU-351e23d4-250b-6a70-2c66-d2fb0221a25a','GPU-ae0edae4-c3bc-cfb2-4b15-f8cdd888bc25']
 text=subprocess.check_output(['nvidia-smi','-i','0,1,2,3,4','--query-gpu=index,uuid,memory.free','--format=csv,noheader,nounits'],text=True);rows=[x.split(',') for x in text.strip().splitlines()]
 require(len(rows)==5 and all(int(r[0])==i and r[1].strip()==expected[i] and float(r[2])>=43000 for i,r in enumerate(rows)),'GPU_MAPPING_OR_FREE_MEMORY');return text
...
def health(row,stage,c):
 ...
 dp=4 if stage==1 else 5;index=read(c[f'stage{stage}']['record_index']);cursor=row['record_cursor'];step=row['update_step'];require(cursor==min(step*dp,len(index)),'STEP_CURSOR')
 require(row['cumulative_tokens']==sum(x['tokens'] for x in index[:cursor]),'GLOBAL_TOKEN_ACCOUNTING')
 seen=[]
 for r in row['rank_runtime']:
  rank=r['rank'];require(rank in range(dp) and r['physical_gpu']==rank and r['weights_finite'] is True,'GPU_OR_FINITE_WEIGHTS')
  want=[x for i,x in enumerate(index[max(0,(step-1)*dp):cursor],start=(step-1)*dp) if i%dp==rank]
 ...
 require(len(seen)==len(set(seen)) and len(row['rank_runtime'])==dp,'DUPLICATE_REAL_RECORD_OR_RANK')
...
def validate_final(root,stage,c):
 ...
 require(a.get('source_authority_sha256')==(c['stage1_parent_source_sha256'] if stage==1 else sha(SOURCE)),'FINAL_SOURCE_PARENT');require(stage!=1 or sha(root/'STAGE1_FINAL_AUTHORITY.json')==c['stage1_parent_authority_sha256'],'IMMUTABLE_STAGE1_AUTHORITY');require(a.get('data_authority_sha256')==sha(c[f'stage{stage}']['data_authority']),'FINAL_DATA_PARENT')

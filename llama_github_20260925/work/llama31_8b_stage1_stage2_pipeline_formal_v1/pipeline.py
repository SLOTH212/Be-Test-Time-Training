...
from core import *
def env(gpus='0,1,2,3'):return {**os.environ,'CUDA_VISIBLE_DEVICES':gpus,'WORLD_SIZE':'4','PYTHONDONTWRITEBYTECODE':'1','OMP_NUM_THREADS':'1','HF_HUB_OFFLINE':'1','TRANSFORMERS_OFFLINE':'1','CUDA_DEVICE_ORDER':'PCI_BUS_ID'}
...
def trainer(stage,c,parent,resume=False):
 ...
 require(journal.exists(),'NO_LAUNCH_INTENT');require(run.exists() if resume else not run.exists(),'TRAINER_ROOT_CONTRACT')
 with lock(root/f'STAGE{stage}.lock'):
  ...
  try:
   ...
   command=[str(R/'envs/ttt_runtime_v1/bin/torchrun'),'--standalone','--nproc-per-node=4','--max-restarts=0',str(W/'entry.py'),'--stage',str(stage),'--config',c[f'stage{stage}']['config'],'--output-dir',str(run)]
  ...
  try:
   while True:
    for stage in (1,2):
     ...
     atomic(R/'provenance/LLAMA31_8B_TWO_STAGE_FINAL_MODEL_INDEX_V1.json',{'FINAL_MODEL_ARTIFACT_N':2,'models':models});move(c,'PIPELINE_COMPLETE','Two FINAL models verified; stop without downstream execution')
     atomic(R/'provenance/LLAMA31_8B_STAGE1_STAGE2_FORMAL_PIPELINE_FINAL_AUTHORITY_V1.json',{'status':'PIPELINE_COMPLETE','models':models,'source_sha256':sha(SOURCE),'bindings':c['bindings'],'state':read(root/'PIPELINE_STATE.json'),'DOWNSTREAM_EXPERIMENTS_AUTO_START':False});return
...
def status(c):
 ...
 result['GPU0_3']=subprocess.check_output(['nvidia-smi','-i','0,1,2,3','--query-gpu=index,memory.used,utilization.gpu','--format=csv,noheader'],text=True)
...
def main():
 ...
 with lock(root/'PIPELINE.lock'):
  ...
  if a.mode=='launch':
   ...
   require(all(not Path(c[f'stage{s}']['run_root']).exists() for s in [1,2]),'FORMAL_RUN_COLLISION');atomic(token,{'source_sha256':sha(SOURCE),'time':time.time(),'execute':True})

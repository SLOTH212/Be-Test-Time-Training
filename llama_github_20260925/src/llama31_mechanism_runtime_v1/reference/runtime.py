...
def dependency(root=None,archive=None):
 c=json.loads((ROOT/'provenance/INFERENCE_RUNTIME_DEPENDENCY.json').read_text())
 root=Path(root or os.environ['TTT_INFERENCE_RUNTIME_ROOT']).resolve()
 archive=Path(archive or os.environ['TTT_INFERENCE_RUNTIME_ARCHIVE']).resolve()
 if sha(archive)!=c['package_sha256']:raise ValueError('INFERENCE_PACKAGE_HASH')
 manifest=root/'provenance/RELEASE_MANIFEST.json'
 if sha(manifest)!=c['release_manifest_sha256']:raise ValueError('INFERENCE_MANIFEST_HASH')
 for p,h in json.loads(manifest.read_text())['files'].items():
  q=(root/p).resolve()
  if not q.is_relative_to(root) or sha(q)!=h:raise ValueError('INFERENCE_SOURCE_HASH '+p)
 return root

def source_functions(relative,names,env=None,chunk=None,integer_overrides=None):
 """Compile named original definitions; never execute reference main/import side effects.

 Only the literal historical chunk size is parameterized when explicitly requested.
 Source hashes are verified against the preserved authority manifest on each load.
 """
 p=ROOT/relative
 authority=json.loads((ROOT/'provenance/MECHANISM_SOURCE_AUTHORITY.json').read_text())
 expected=next(x['sha256'] for x in authority['entries'] if x['packaged_path']==relative)
 if sha(p)!=expected:raise ValueError('MECHANISM_SOURCE_HASH '+relative)
 tree=ast.parse(p.read_text());nodes=[copy.deepcopy(x) for x in tree.body if isinstance(x,(ast.FunctionDef,ast.ClassDef)) and x.name in names]
 if {x.name for x in nodes}!=set(names):raise ValueError('SOURCE_DEFINITION_SET')
 if chunk is not None or integer_overrides:
...
def reset_rules(layers):
 return source_functions('provenance/original/reset/screening/scripts/screen_state_reset.py',['historical_early_steady','terminal_run_boundary'],{'ACTIONS':actions(layers)})
def deletion_rules(layers):
 return source_functions('provenance/original/deletion/rule_sources/build_ablation_manifest.py',['category','first_real_layer'],{'ACTION_LAYERS':mapping(layers)})
...
def r2_winner(records,layers):
 return source_functions('provenance/original/r2/scripts/aggregate_exact_r2.py',['independent_winner'],{'ACTIONS':actions(layers),'TOL':TOL}).independent_winner(records)

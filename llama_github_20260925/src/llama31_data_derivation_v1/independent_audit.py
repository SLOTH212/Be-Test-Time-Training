...
R=Path("/path/to/ttt")
S1P=R/"datasets";S2P=R/"shared/datasets"
...
def smoke_audit():
    tok=AutoTokenizer.from_pretrained(R/"models/Llama-3.1-8B",local_files_only=True,use_fast=True,trust_remote_code=False)
    samples=list(rows(R/"work/llama31_derived_data_v1/smoke/SMOKE_SAMPLES.jsonl"));bad=0;sources={1:set(),2:set()};multi=False
    for x in samples:
        sources[x["stage"]].add(x["source"])
        if x["stage"]==2:ids,ranges=independent_answer_ranges(tok,x["text"]);bad+=ranges!=x["answer_ranges"];multi|=x["qa_count"]>1
        else:ids=tok.encode(x["text"],add_special_tokens=False)
        bad+=len(ids)!=x["llama_token_count"] or token_hash(ids)!=x["llama_token_sha256"]
    out={"status":"PASS" if not bad and len(sources[1])>=3 and len(sources[2])==5 and multi else "FAIL","independent_llama_retokenization":"PASS" if not bad else "FAIL","stage1_source_n":len(sources[1]),"stage2_source_n":len(sources[2]),"stage2_multi_answer_exercised":multi,"sample_n":len(samples)}
    atomic(R/"work/llama31_derived_data_v1/smoke/LLAMA_DATA_BUILDER_INDEPENDENT_SMOKE.json",out)
    if out["status"]!="PASS":raise SystemExit(1)
    print(json.dumps(out,sort_keys=True));return

def main(stage):
    if stage=="smoke":return smoke_audit()
    if stage=="stage1":

"""Original Llama-3.1-8B, frozen raw RULER prompts, no TTT or trained parent."""
import os,sys,json,time,statistics,collections,subprocess,fcntl,argparse
from pathlib import Path
R=Path('/path/to/ttt');S=R/'src/llama31_base_ruler32k_v1';C=R/'configs/llama31_base_ruler32k_v1.json'
sys.path.insert(0,str(R/'src/llama31_fixed8_formal_v1'));import runner as util
def config(full=False):
 ...
 assert c['model_path']==str(R/'models/Llama-3.1-8B') and c['ttt_enabled'] is False and c['excluded_tasks']==[]
 return c
def worker(args):
 ...
 assert len(model.model.layers)==32 and not any('ttt_' in n for n in model.state_dict())
 ...
 for i,row in enumerate(rows,1):
  ...
  enc=tok(row['prompt'],return_tensors='pt',add_special_tokens=True,truncation=False);n=enc.input_ids.shape[1];assert n==row['prompt_tokens'] and n+row['max_new_tokens']<=32768

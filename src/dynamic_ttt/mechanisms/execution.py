...
CLI without --plan. Original tensor operations are compiled unchanged;
...
def runner(cfg,dep):
 p=dep/'dynamic/search.py';tree=ast.parse(p.read_text())
...
def backend(cfg):
 ...
 return rt.source_functions('kernels/reset_backend.py',names,{'LAYERS':cfg['configured_ttt_layers'],'ACTION_LAYERS':rt.mapping(cfg['configured_ttt_layers'])})

def kp_modules(cfg,dr,model):
 import torch
 import torch.nn.functional as F
 from dynamic_ttt.models.inference.hf_qwen3.modeling_qwen3 import TTTDynamicCache
 from dynamic_ttt.ttt.qwen_state import TTTState,TTTStats,ttt_chunk_step,build_chunk_local_ntp_pairs
 ...
 c=rt.source_functions('kernels/closure.py',['th','base','PrefixKVInjectCache','install_formal','configure','restore','run_branch','donor_prefix'],env,chunk=cfg['chunk_size'])
 f=rt.source_functions('kernels/factorial.py',['run_prompt','run_reward'],{'torch':torch,'c':c,'LAYERS':cfg['configured_ttt_layers'],'base':c.base,'th':c.th},chunk=cfg['chunk_size'])
 w=rt.source_functions('kernels/trace.py',['cmp','Recorder','prefix_hashes','first_divergence'],{},chunk=cfg['chunk_size'],integer_overrides={28:len(model.model.layers)})
...
class Executor:
 def __init__(self,cfg,benchmark_rows,fixed,output_root):
  ...
  p=self.dep/'eval/scoring.py';ns={'__name__':'mechanism_scorer'};exec(compile(p.read_text(),str(p),'exec'),ns)

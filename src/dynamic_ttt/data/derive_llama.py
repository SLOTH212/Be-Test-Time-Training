...
ROOT = Path(os.path.expandvars("${OUTPUT_ROOT}"))
SRC = ROOT / "src/llama31_data_derivation_v1"
WORK = ROOT / "work/llama31_derived_data_v1"
LOG = ROOT / "logs/llama31_derived_data_v1"
PROV = ROOT / "provenance"
S1P = Path(os.environ["DATA_ROOT"]) / "qwen_stage1"
S2P = Path(os.environ["DATA_ROOT"]) / "qwen_stage2"
S1 = ROOT / "datasets/llama31_stage1_500m_32k_v1"
S2 = ROOT / "datasets/llama31_stage2_15m_32k_v1"
TOK = Path(os.environ["MODEL_ROOT"]) / "Llama-3.1-8B"
QWEN = Path(os.environ["MODEL_ROOT"]) / "Qwen3-4B-Base"
MAXLEN = 32768
...
def hashes_under(root):
    excluded={"authority/DATASET_AUTHORITY.json","audits/PACKAGE_VERIFY.json"}
    return {p.relative_to(root).as_posix():sha_file(p) for p in sorted(root.rglob("*")) if p.is_file() and p.relative_to(root).as_posix() not in excluded}

...
class TTTDynamicCache(DynamicCache):
    def __init__(self, ddp_cache_data=None, config=None) -> None:
        ...
        self.ttt_states = [(None, None, None)] * config.num_hidden_layers  # (past_h, past_t, past_w) per layer
...
class LlamaMLP(nn.Module):
    ...
    def forward(self, x, t=None, past_w=None, enabled=True):  # TTT: explicit runtime switch
        ...
        present_down_proj_w = self.down_proj.weight if past_w is None else past_w
        if t is None:
        ...
        outputs = []
        state = TTTState(present_down_proj_w)
        if full_len:
            ...
            full_out, state, stats = run_document(
                h[:, :full_len], t[:, :full_len], self.down_proj.weight,
                projection, self.ttt_conv.weight,
                torch.ones((1, full_len), dtype=torch.bool, device=x.device),
                chunk_size=self.ttt_chunk, learning_rate=self.ttt_lr,
                enabled=True, delta_clip_norm=self.ttt_update_clip_norm,
                initial_state=state, target_type=getattr(self, "ttt_target_type", "conv"),
                target_gate=getattr(self, "ttt_ntp_gate", None))
            outputs.append(full_out)
            self.last_ttt_stats = stats
        if full_len < x.shape[1]:
            outputs.append(nn.functional.linear(h[:, full_len:], state.weight, self.down_proj.bias))
...
class LlamaDecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        ttt_generation_step: bool = False,
        **kwargs: Unpack[TransformersKwargs],
    ) -> torch.Tensor:
        ...
        if ttt_enabled and target_states is None and hasattr(self.mlp, "ttt_conv"):
            target_states = hidden_states
        # TTT: branch on whether this layer has TTT target states
        # Frozen tailfix: generation applies the prompt-derived fast weight but
        # never appends tokens to the prompt tail or produces online updates.
        if ttt_generation_step:
            target_states = None
        if target_states is None:
            if ttt_enabled and hasattr(self.mlp, "ttt_conv") and past_key_values is not None:
                _, _, past_w = past_key_values.ttt_states[self.layer_idx]
                hidden_states, present_w = self.mlp(hidden_states, None, past_w)
                past_key_values.TTT_update((None, None, present_w), self.layer_idx)
            else:
                hidden_states = self.mlp(hidden_states, enabled=ttt_enabled)
        else:

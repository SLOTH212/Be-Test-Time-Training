...
from dynamic_ttt.ttt.state import GATED_NTP_TARGET, TTTState, run_document, run_sequence_with_boundaries
...
class LlamaMLP(nn.Module):
    def __init__(self, config, layer_idx: Optional[int] = None):  # TTT: added layer_idx
        ...
        if getattr(config, "ttt_mode", False) and self.layer_idx in getattr(config, "ttt_layers", []):
            ...
            self.ttt_target_type = getattr(config, "ttt_target_type", "conv")
            if self.ttt_target_type == GATED_NTP_TARGET:
                self.ttt_ntp_gate = nn.Parameter(torch.zeros(self.hidden_size))
            self.ttt_update_clip_norm = getattr(config, "ttt_update_clip_norm", None)
            self.ttt_conv = nn.Conv1d(
                self.hidden_size, self.hidden_size, kernel_size=5, padding=2,
                groups=self.hidden_size, bias=False,
            )

    def _clip_ttt_update(self, delta):
        tau = self.ttt_update_clip_norm
        if tau is None:
            return delta

        norm = torch.linalg.vector_norm(
            delta.float(), dim=(-2, -1), keepdim=True
        )
        eps = torch.finfo(norm.dtype).tiny
        scale = (tau / norm.clamp_min(eps)).clamp(max=1.0)
    ...
    def forward(self, x, t: Optional[torch.Tensor] = None, valid_mask=None, document_boundaries=None):
        ...
        target_type = getattr(self, "ttt_target_type", "conv")
        if target_type == GATED_NTP_TARGET and self.ttt_proj is None:
            raise ValueError("Gated In-Place NTP requires the preserved ttt_proj")
        if target_type == GATED_NTP_TARGET and getattr(
            self.config, "ttt_document_metadata_required", True
        ) and document_boundaries is None:
            raise ValueError("Gated In-Place NTP requires explicit document_boundaries")
        output, stats = run_sequence_with_boundaries(
            h, t, self.down_proj.weight, projection, self.ttt_conv.weight,
            valid_mask, document_boundaries=document_boundaries,
            chunk_size=self.ttt_chunk, learning_rate=self.ttt_lr,
            enabled=True, delta_clip_norm=None, target_type=target_type,
            target_gate=getattr(self, "ttt_ntp_gate", None))
...
class LlamaDecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        hidden_states = self.mlp(hidden_states, t=target_states, valid_mask=ttt_valid_mask, document_boundaries=document_boundaries)
        hidden_states = residual + hidden_states
...
class LlamaPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        # Raw parameters are not covered by the Linear/Conv module branches.
        # Re-zero a gate materialized from meta when it is missing in Base.
        if hasattr(module, "ttt_ntp_gate") and module.ttt_ntp_gate.device.type != "meta":
            module.ttt_ntp_gate.data.zero_()
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if self.ttt_mode:
            # Resolve the canonical token-valid mask once from the original 2-D mask.
            if attention_mask is None:
                ttt_valid_mask = torch.ones(hidden_states.shape[:2], dtype=torch.bool, device=hidden_states.device)
            elif attention_mask.ndim == 2:
                if attention_mask.shape != hidden_states.shape[:2]:
                    raise ValueError("ttt_valid_mask shape must match [batch, sequence]")
                ttt_valid_mask = attention_mask.to(torch.bool)
            else:
                raise ValueError("TTT requires the original 2-D attention_mask, not an expanded attention bias")
            # Consume and normalize document boundaries exactly once.
            if document_boundaries is not None:
                document_boundaries = [(int(a), int(b)) for a, b in document_boundaries]

...
_CACHE_ROOT = Path(os.environ.get("OUTPUT_ROOT", ".")).resolve()
...
from dynamic_ttt.training.lib.distributed_checkpoint import (DistributedCheckpointRotation, load_model_only,
                                        preflight_checkpoint_metadata, save_model_only)
from dynamic_ttt.training.lib.distributed_grad import clip_grad_norm_fsdp2
from dynamic_ttt.training.lib.distributed_runtime import barrier, initialize, rank0_mkdir, shutdown
from dynamic_ttt.training.lib.distributed_semantics import BatchPlan, assert_batch_equation, differentiable_empty_step
from dynamic_ttt.training.lib.fsdp2_runtime import apply_fsdp2, make_device_mesh
from dynamic_ttt.training.lib.model_factory import build_pretrained
from dynamic_ttt.training.lib.pipeline_core import atomic_json, load_yaml
...
def stage1_document_boundaries(row, token_count, input_ids=None):
    ...
    for left, right in zip(bounds, bounds[1:]):
        ...
        if left[1] < right[0]:
            ...
            if gap.numel() != 1 or int(gap.item()) != 128001:
...
def validate_stage1(model, tokenizer, path, limit, context):
    ...
    with torch.no_grad():
        for window in windows(Path(path), 0, limit, context.world_size):
            ...
            ids = torch.tensor([row["input_ids"]], dtype=torch.long, device=context.device)
...
def main():
    ...
    if os.environ.get("CUDA_VISIBLE_DEVICES") != str(cfg["physical_gpu_allowlist"]):
        raise RuntimeError("PREINIT_PHYSICAL_GPU_ALLOWLIST_VIOLATION")
    if int(os.environ.get("WORLD_SIZE", "0")) != 4 or int(os.environ.get("LOCAL_RANK", "-1")) not in range(4):
    ...
    try:
        ...
        for bound_path,bound_hash in cfg['dataset_file_bindings'].items():
        ...
        if parameter_count != 8131072000 or len(model.model.layers) != 32:
            raise RuntimeError("LLAMA31_8B_TTT_MODEL_PARAMETER_IDENTITY")
        mesh = make_device_mesh(context.device.type, context.world_size)
        inventory = apply_fsdp2(model, mesh=mesh)
        if inventory.decoder_blocks != 32 or not all(hasattr(block, "set_requires_gradient_sync") for block in model.model.layers):
            raise RuntimeError("FORMAL_FSDP2_OWNERSHIP")
        if context.is_rank0:
            atomic_json(output / "model_build.json", {"parameter_count":parameter_count,"layers":32,
        ...
        while True:
            ...
            for slot in range(plan.gradient_accumulation):
                ...
                if loss is None:
                    ...
                    dummy = window[0][1]
                    dummy_ids = torch.tensor([dummy["input_ids"]], dtype=torch.long, device=context.device)
            ...
            if not math.isfinite(global_mean_loss):
            ...
            eligible_ttt_update = any(any(int(b)-int(a) > int(cfg["ttt_chunk_size"])
                for a,b in (json.loads(row["document_boundaries_qwen_json"])
                            ...
                            else row["document_boundaries_qwen_json"])) for _,row in window)
            l30_gradient = {}
            ...
            local_runtime = {
                ...
                "local_tokens": sum(int(row["token_count"]) for pos,row in window if plan.owner(pos)[2] == context.rank),
...
from dynamic_ttt.data.record_adapter import windows, answer_ranges

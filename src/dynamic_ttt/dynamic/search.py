...
def load_model(ctx):
    ...
    from dynamic_ttt.models.selection import inference_classes
    Qwen3Config,Qwen3ForCausalLM=inference_classes(getattr(ctx,"config",{"model_family":"qwen3","ttt_chunk_size":1024}))
...
def install_dynamic_forward(model):
    ...
    from dynamic_ttt.ttt.qwen_state import TTTState,TTTStats,ttt_chunk_step
    for layer in LAYERS:
...
class Evaluator:
    ...
    def evaluate(self,sequence,capture=False,force=False):
        ...
        used_layers=set(x for a in seq for x in ACTION_LAYERS[a])
        for layer in LAYERS:
        ...
        # A layer never selected anywhere in this sequence is executed through
        # the exact formal non-TTT decoder/MLP path.  This is semantically
        # identical to an always-disabled update mask, while also preserving
        # bitwise constant-mode replay against models instantiated with only
        # the selected formal layer modules.
        stashed={}
        for layer in LAYERS:
            if layer in used_layers:continue
            m=self.model.model.layers[layer].mlp;stashed[layer]={}
            for name in ['ttt_conv','ttt_proj','ttt_ntp_gate']:
                if hasattr(m,name):stashed[layer][name]=getattr(m,name);delattr(m,name)
        previous_ttt_mode=self.model.model.ttt_mode
        if not used_layers:self.model.model.ttt_mode=False
        ...
        try:
            ...
            self.model.model.ttt_mode=previous_ttt_mode
            for layer,attrs in stashed.items():
                m=self.model.model.layers[layer].mlp
                for name,value in attrs.items():setattr(m,name,value)
        ...
        for layer in LAYERS:
            ...
            if layer not in used_layers:
                state_hash=tensor_hash(m.down_proj.weight) if capture else None
                ev=[{'layer':layer,'chunk_index':j+1,'selected_action':seq[j],'active_update':False,
                    'pre_state_identity':f'{branch}:L{layer}:v0','post_state_identity':f'{branch}:L{layer}:v0',
                    'pre_state_hash':state_hash,'post_state_hash':state_hash,'post_clip_delta_norm':0.0,
                    'subsequent_recomputation':True} for j in range(self.full_chunks)]
                events.extend(ev)
        ...
        if c and (c==1 or c%10==0):
            ...
            progress={'stage':self.stage,'completed_samples':self.sample_ordinal-1,'total_samples':self.total_samples,
                ...
                'sample_best_score':max(float(self.ctx.baseline[a][self.row['sample_id']]['official_sample_score']) for a in ACTIONS),
...
def search_sample(ctx,model,tok,row,stage,ordinal,total,progress):
    ...
    m=morphology(chosen['sequence']);result={'stage':stage,'task':row['task'],'sample_id':row['sample_id'],'sample_index':row['_sample_index'],
        'full_chunk_count':ev.full_chunks,'fixed_scores':scores,'sample_best_score':fixed,'best_fixed_actions':best_actions,

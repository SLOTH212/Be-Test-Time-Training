...
from dynamic_ttt.eval.runtime import action_layers, atomic_json, fixed_actions, load_config, sha256, validate_benchmark_authority, validate_model_authority
...
def execute(cfg):
    ...
    Qwen3Config,Qwen3ForCausalLM=inference_classes(cfg)
    scorer_path = ROOT / "eval/scoring.py"
        ...
        try:
            for index,(task,row) in enumerate(rows):
                ...
                if output.exists():
                    ...
                    if saved.get('config_identity')!=config_id or saved.get('sample_id')!=row['sample_id'] or saved.get('action')!=action or saved.get('status')!='SUCCESS':
                        raise ValueError('INVALID_FIXED_RESUME_RESULT')
                    continue

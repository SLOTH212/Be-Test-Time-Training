...
def install_dynamic_forward(model):
    ...
    for layer in LAYERS:
        ...
        def patched(self,x,t=None,past_w=None,enabled=True):
            ...
            if _suite_trace is not None:_suite_trace.enter(self,x,h,t,past_w)
            ...
            if t is None:
                ...
                result=F.linear(h,present,self.down_proj.bias)
                if _suite_trace is not None:_suite_trace.generation(self,x,h,result,present)
            ...
            for j,start in enumerate(range(0,full_len,self.ttt_chunk)):
                ...
                entry_weight=state.weight
                if _suite_trace is not None:_suite_trace.location(int(self.layer_idx),j+1)
                if active:
                ...
                if _suite_trace is not None:_suite_trace.chunk(self,j+1,x[:,start:end],h[:,start:end],out,entry_weight,state.weight,stat,active)
                outputs.append(out);stats.append(stat)
            if full_len<x.shape[1]:
                tail_out=F.linear(h[:,full_len:],state.weight,self.down_proj.bias)
                if _suite_trace is not None:_suite_trace.tail(self,x[:,full_len:],h[:,full_len:],tail_out,state.weight)

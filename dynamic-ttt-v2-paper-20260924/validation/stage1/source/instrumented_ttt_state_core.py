...
def ttt_chunk_step(hidden, target_source, base_weight, target_projection,
    ...
    if _suite_trace is not None:_suite_trace.proposal("target_v",target)
    mask = update_valid.to(hidden.dtype).unsqueeze(-1)
    delta = torch.einsum(
        "bti,btd,de->ei", update_hidden * mask, target * mask, target_projection
    ) * learning_rate
    if _suite_trace is not None:_suite_trace.proposal("raw_delta",delta)
    ...
    if _suite_trace is not None:_suite_trace.proposal("clipped_delta",delta)
    next_weight=current if (_suite_trace is not None and _suite_trace.force_zero) else current + delta
    return output, TTTState(next_weight), TTTStats(
...
def run_document(hidden, target_source, base_weight, target_projection,
    ...
    if chunk_size != 1024 and not allow_test_override:
        raise ValueError("frozen Static-TTT chunk_size must be 1024")
    if enabled and hidden.shape[0] != 1:
        raise ValueError("TTT-on currently requires batch_size=1 because fast weights are per sample")

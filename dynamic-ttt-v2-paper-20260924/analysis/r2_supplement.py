...
import numpy as np
# Portable adapter: original numerical formulas below are unchanged.
# R2 input bytes are package JSONL serializations, not original pretty-printed files.
import argparse,gzip
ROOT=Path(__file__).resolve().parents[1]
parser=argparse.ArgumentParser();parser.add_argument('--out',type=Path,default=ROOT/'outputs/r2_supplement');args=parser.parse_args()
P=args.out.resolve();P.mkdir(parents=True,exist_ok=True)
BASE=ROOT/'analysis/archival/r2';R=P/'packaged_inputs';(R/'r2').mkdir(parents=True,exist_ok=True)
(R/'COHORT.json').write_bytes((ROOT/'experiments/mechanism/COHORT.json').read_bytes())
package_prior={}
with gzip.open(ROOT/'data/mechanism/r2.jsonl.gz','rt') as stream:
 for line in stream:
  rec=json.loads(line);key=hashlib.sha256(rec['sample_id'].encode()).hexdigest();p=R/'r2'/f'{key}.json'
  p.write_text(json.dumps(rec,separators=(',',':'))+'\n');package_prior[str(p)]={'sha256':hashlib.sha256(p.read_bytes()).hexdigest()}
assert len(package_prior)==170
...
cohort=j(R/'COHORT.json');cp={r['sample_id']:r for r in cohort}; prior=package_prior;data={};sets=[];mass=[];pairnull=[];checks=Counter()
...
# Compare all reproduced numerical tables with the frozen completed audit.
verified=[]
for name in ['setwise_alignment_and_reversal.csv','setwise_summary.csv','task_transition_enrichment.csv','task_transition_types.csv','dominant_transition_by_task.csv','cross_task_direction_consistency.csv','within_task_rank_correlations.csv']:
 a=list(csv.DictReader((P/name).open()));b=list(csv.DictReader((BASE/'completion'/name).open()));assert len(a)==len(b),name
 for x,y in zip(a,b):
  assert set(x)==set(y)
  for k in x:
   if x[k]==y[k]:continue
   try:assert abs(float(x[k])-float(y[k]))<1e-10,(name,k,x[k],y[k])
   except ValueError:raise AssertionError((name,k,x[k],y[k]))
 verified.append(name)
save('REPRODUCTION_CHECK.json',{'status':'PASS','verified_tables':verified,'GPU_USED':False,'MODEL_LOADED':False,'input_identity':'package serialized R2; original hashes remain in provenance/COPIED_FILES.json','scope':'set-wise alignment/reversal CIs, task direction and task-centered rank correlations; base per-sample structure table supplied as frozen input'})
print('R2 supplementary tables reproduced: PASS',flush=True)
